...
export async function POST(request: NextRequest) {
  ...
  if (!email || !token) {
    return NextResponse.json({ ok: false, message: "Preencha e-mail e token." }, { status: 400 });
  }

  const user = await findUserByCredentials(email, token);

  if (!user) {
    return NextResponse.json({ ok: false, message: "Credenciais não encontradas na base." }, { status: 401 });
  }
