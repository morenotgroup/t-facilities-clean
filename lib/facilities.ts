...
const DEFAULT_KIT_BY_CATEGORY: Record<string, string[]> = {
  ...
  Escritório: [
    ...
    "Multiuso neutro",
    "Mop úmido",
    "Álcool 70% para pontos críticos",
    "Saco de lixo"
  ],
  Operacional: [
    "Pano microfibra",
...
const DEFAULT_EPI_BY_CATEGORY: Record<string, string[]> = {
  ...
  Operacional: [
    ...
    "Óculos de proteção em uso químico"
  ],
  Almoço: [
    "Touca",
    "Avental",
...
export async function buildDashboardForSession(session: SessionPayload) {
  const [collaborators, ambientes, logs] = await Promise.all([
    getCollaborators(),
    getAmbientes(),
    getLogs()
  ]);
...
export async function getRouteByInstanceForUser(
  session: SessionPayload,
  instanceId: string
) {
  const [collaborators, ambientes] = await Promise.all([
    getCollaborators(),
    getAmbientes()
  ]);
  ...
  return (
    buildRoutesForDay({
      ...
      collaborator,
      date: new Date()
    }).find((route) => route.instanceId === instanceId) || null
  );
}

export async function getRouteDetailForSession(
  session: SessionPayload,
  instanceId: string
) {
...
export async function buildLeaderDashboard() {
  const [collaborators, logs, feedbacks] = await Promise.all([
    getCollaborators(),
    getLogs(),
    getFeedbacks()
  ]);
